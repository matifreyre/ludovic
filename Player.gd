...
func _ready():
	# Todavía no sé para qué está esto
#	set_process(true)
	pass

func _process(delta):
	# Seguir el movimiento del mouse mientras se esté arrastrando
	if is_grabbed:
...
# TODO Debe haber una mejor forma de acotar el espacio y hacer la transación
func move(direction):
	var new_coordinates = coordinates - direction
	var new_position = new_coordinates * CELL_SIZE
