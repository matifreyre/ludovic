...
func _physics_process(delta : float):
	if has_turn:
		# has_turn = false # acá no sirve, jode detección del input
...
# Drag and drop
func _on_PlayerArea_input_event(viewport : Viewport, event : InputEvent, shape_idx : int):
...
func get_next_coordinates(direction : Vector2):
	return self.clamped_coordinates(coordinates + direction)


func clamped_coordinates(coords : Vector2):
...
func set_coordinates(new_coordinates : Vector2):
...
func get_position_for(coords : Vector2):  
...
func transition_movement_to(new_position : Vector2):
